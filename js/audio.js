...
VA.Audio = {
  ...
  _synth: {
    ...
    ting()   { this._tone({ f0: 2080, f1: 2750, dur: .18, type: 'triangle', gain: .27 }); },
    spike()  { this._tone({ f0: 310, f1: 58, dur: .32, type: 'sawtooth', gain: .54 }); this._noise({ dur: .28, freq: 1250, gain: .44, type: 'bandpass', q: .7 }); this._noise({ dur: .42, freq: 520, gain: .2, at: .05 }); },
    boom()   { this._tone({ f0: 88, f1: 29, dur: .62, type: 'sine', gain: .7 }); this._noise({ dur: .55, freq: 250, gain: .25 }); },
    rumble() { this._tone({ f0: 72, f1: 36, dur: .72, type: 'sine', gain: .44 }); this._noise({ dur: .62, freq: 190, gain: .18 }); },
    debris() { [0, .13, .29].forEach((at, i) => this._noise({ dur: .055, freq: 1200 - i * 210, gain: .2, at, type: 'bandpass', q: 1.1 })); },
